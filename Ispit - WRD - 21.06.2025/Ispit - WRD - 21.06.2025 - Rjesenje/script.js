...
let k5_posalji = () => {
  ...
  if (!provjeriIme()) {
    messageSuccess("Ime neispravno");
    return;
  }
  if (!provjeriPasos()) {
    messageSuccess("Broj pasosa neispravan");
    return;
  }
  if (!provjeriEmail()) {
    messageSuccess("Email neispravan");
    return;
  }
...
let provjeriIme = () => {
  const ime = document.getElementById('gosti');
  const input = ime.querySelectorAll('input');
...
let provjeriPasos = () => {
if (!/^[A-Z]{3}[0-9][A-D]{2}[0-9]-[1-5]$/.test(document.getElementById("brojPasosa").value) ||document.getElementById("brojPasosa").value == "") {
 ...
 */
let provjeriEmail = () => {
if (!/^[a-z]+[\.-][a-z]+@(gmail.com|edu.fit.ba)$/.test(document.getElementById('email').value) ||document.getElementById('email').value == "") {
